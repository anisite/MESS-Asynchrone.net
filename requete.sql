     ...
     AS (SELECT [CA_NM_LIBL_SERV_ASYN], [CA_D_HR_EXEC_PLAN], [CA_E_REQT], [CA_C_TYPE_REQT_ASYN],
                Row_number()
                  OVER( --OVER permet d'ajouter des numéros de lignes sur la partition (équivalent d'un group by)
                    PARTITION BY CASE WHEN [R].[CA_C_MODE_REQT_CONT] = 'AvecConvoi' THEN [R].[CA_N_IDEN_CONV] -- Numéro applicatif du convoi
                                      WHEN [R].[CA_C_MODE_REQT_CONT] IS NULL THEN CONVERT(NVARCHAR(50), [R].[CA_N_IDEN_REQT])
