                ...
                Row_number()
                  OVER( --OVER permet d'ajouter des numéros de lignes sur la partition (équivalent d'un group by)
                    PARTITION BY CASE WHEN [R].[CA_C_MODE_REQT_CONT] = 'AvecConvoi'
                                    ...
                                    ELSE [R].[CA_N_EVEN_AFFR_RECP] + CONVERT(varchar(3),ABS(CHECKSUM(NEWID())) % {largeurParEvenementAffaire}) -- Dans le cas d'un traitement SansConvoi, crée un convoi éphémère
                                 END
