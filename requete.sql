     ...
     AS (SELECT [CA_NM_LIBL_SERV_ASYN], [CA_D_HR_EXEC_PLAN], [CA_E_REQT], [CA_C_TYPE_REQT_ASYN],
                Row_number()
                  OVER( --OVER permet d'ajouter des numéros de lignes sur la partition (équivalent d'un group by)
                    PARTITION BY CASE WHEN [CA_C_MODE_REQT_CONT] = 'AvecConvoi' THEN [CA_N_IDEN_CONV] -- Numéro applicatif du convoi
                                      WHEN [CA_C_MODE_REQT_CONT] IS NULL THEN CONVERT(NVARCHAR(50), [CA_N_IDEN_REQT])
                                      ELSE [CA_N_EVEN_AFFR_RECP] + CA_E_REQT + CONVERT(varchar(3),ABS(CHECKSUM(NEWID())) % {maximumParEvenementAffaire}) -- Dans le cas d'un traitement SansConvoi, crée un convoi éphémère
                                 END
                    ORDER BY[CA_D_INSC_REQT] ) AS NumeroLigne
         FROM [dbo].[CA1_REQUETE_ASYNCHRONE]
